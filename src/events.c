...
#include "guitar_hero_drums.h" /* for guitar hero drums */
...
void handshake_expansion(struct wiimote_t *wm, byte *data, uint16_t len)
{
    ...
    case EXP_ID_CODE_DRUMS:
        if (guitar_hero_drums_handshake(wm, &wm->exp.ghdrums, handshake_buf, EXP_HANDSHAKE_LEN))
        {
...
static void save_state(struct wiimote_t *wm)
{
    ...
    case EXP_GUITAR_HERO_DRUMS:
        /* TODO */
        wm->lstate.exp_ljs_ang    = wm->exp.ghdrums.js.ang;
        wm->lstate.exp_ljs_mag    = wm->exp.ghdrums.js.mag;
        wm->lstate.exp_r_shoulder = wm->exp.ghdrums.whammy_bar;
        wm->lstate.exp_btns       = wm->exp.ghdrums.btns;
        break;
...
#define CROSS_THRESH_XYZ(last, now, thresh)                                            \
    ...
    {
        // TODO
        STATE_CHANGED(wm->lstate.exp_ljs_ang, wm->exp.ghdrums.js.ang);
        STATE_CHANGED(wm->lstate.exp_ljs_mag, wm->exp.ghdrums.js.mag);
        STATE_CHANGED(wm->lstate.exp_r_shoulder, wm->exp.ghdrums.whammy_bar);
        STATE_CHANGED(wm->lstate.exp_btns, wm->exp.ghdrums.btns);
        break;
    }
